...
#include <sys/select.h> // select() — inti async I/O
#include <time.h>       // timestamp untuk logging
...
#define MAX_CLIENTS 64        // maksimal klien terhubung sekaligus
#define BUFFER_SIZE 1024      // ukuran buffer baca data
#define ADMIN_PASSWORD "protocol7"
#define LOG_FILE "history.log"
```
Fungsinya: Mendaftarkan semua library sistem yang dibutuhkan dan mendefinisikan konstanta agar mudah diubah di satu tempat.

```
typedef struct {
    int fd;           // file descriptor koneksi (nomor unik tiap klien)
    char name[64];    // nama pengguna
    int is_admin;     // 1 jika The Knights, 0 jika user biasa
    int authenticated; // 0=belum ada nama, 1=aktif, 2=tunggu password
} Client;

Client clients[MAX_CLIENTS]; // array semua klien yang terhubung
int client_count = 0;        // jumlah klien saat ini
time_t server_start;         // waktu server mulai (untuk RPC uptime)
```
Fungsinya: Blueprint data tiap klien. Semua info klien tersimpan di sini — seperti "kartu identitas" tiap koneksi.

```
void log_entry(const char *role, const char *msg) {
    FILE *f = fopen(LOG_FILE, "a");  // buka history.log mode append
    ...
    fprintf(f, "[2026-04-26 19:06:40] [System] [SERVER ONLINE]\n");
    fclose(f);
}
```
Fungsinya: Setiap kali ada event penting (user connect, pesan dikirim, admin command), fungsi ini dipanggil untuk mencatat ke history.log dengan format timestamp yang ditentukan soal.

```
find_client(fd) = Cari index klien di array berdasarkan
fd-nyaname_exists(name) = Cek apakah nama sudah dipakai klien lain
send_to(fd, msg) = Kirim pesan ke satu klien tertentu
broadcast(msg, exclude_fd) = Kirim pesan ke semua klien kecuali pengirimnya
```
```
void remove_client(int fd) {
    // 1. Broadcast notif "[System] [User 'alice' disconnected]"
    // 2. close(fd) — tutup koneksi
    // 3. Timpa slot klien dengan klien terakhir di array
    //    (cara efisien menghapus dari array tanpa geser semua)
    client_count--;
}
```
Fungsinya: Membersihkan klien yang disconnect — baik karena /exit, Ctrl+C, maupun koneksi putus tiba-tiba.

```
if (cmd == 1) { /* RPC_GET_USERS  → hitung & kirim jumlah user aktif */ }
if (cmd == 2) { /* RPC_GET_UPTIME → hitung time(NULL) - server_start  */ }
if (cmd == 3) { /* RPC_SHUTDOWN   → broadcast shutdown, exit(0)       */ }
if (cmd == 4) { /* Disconnect     → remove_client()                    */ }
```
Fungsinya: Menu khusus The Knights. Hanya dipanggil ketika klien sudah terbukti admin (is_admin == 1)

```
authenticated == 0  →  klien baru, minta nama
authenticated == 2  →  nama "The Knights" sudah masuk, minta password
authenticated == 1  →  klien aktif:
    - is_admin == 1  →  tampilkan menu RPC, proses pilihan
    - is_admin == 0  →  broadcast pesan chat / proses /exit
```
```
1. Baca file "protocol" → dapat IP & port
2. Buat server socket → bind → listen
3. Log "SERVER ONLINE"
4. Loop tak terbatas:
   ├── select() → tunggu aktivitas di semua fd sekaligus
   ├── Jika server_fd aktif → accept() koneksi baru
   └── Jika client fd aktif → handle_client_data()
```
select() adalah kunci utama: ia memantau puluhan koneksi sekaligus tanpa fork — server tidak perlu membuat proses/thread baru per klien. Ketika ada data masuk di salah satu fd, select() "membangunkan" server dan langsung diproses.
